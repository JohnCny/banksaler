function getAllCompanyCredit(){
    var get = crud.dom.factory("GET");
    var url = "/credit/list?credit_type=2";
    ...
    function getAllUserCallback(json){
        ...
        for(var i = 0;i<objs.length;i++){
            var content = '<div>'+
                '<a class="btn btn-info" href="javascript:void(0);" onclick="gotoBjCompanyCredit('+objs[i].id+','+objs[i].customer_id+');">'+
                    '<i class="icon-edit icon-white"></i>编辑'+
                '</a>&nbsp;'+
                '<a class="btn btn-danger" href="javascript:void(0);" onclick="deleteCompanyCredit('+objs[i].id+');">'+
                    '<i class="icon-trash icon-white"></i>删除'+
                '</a>'+ 
            '</div>';
            $('#yhTable').dataTable().fnAddData( [
              '<input type="checkbox" name="company_credit_checkbox" onclick="changeColor(this)"/>',
...
function gotoBjCompanyCredit(id,customer_id){
    var param = {
        "id":id,
...
function initCompanyCreditContent(){
...
function deleteCompanyCredit(id){
    var del = crud.dom.factory("DELETE");
    var url = "/credit/" + id;
    del.doDelete(url,initSalerContentCallback,"删除失败！");
    
    function initSalerContentCallback(json){
        window.location.href = "company_credit";
    }
}

function getAllCompanyCreditGrant(){
    var get = crud.dom.factory("GET");
    var url = "/grantcredit/list?credit_type=2";
    get.doGet(url,getAllUserCallback,"加载贷款信息失败！");
    ...
    function getAllUserCallback(json){
        ...
        for(var i = 0;i<objs.length;i++){
            var content = '<div>'+
                '<a class="btn btn-info" href="javascript:void(0);" onclick="gotoBjCompanyCreditGrant('+objs[i].id+','+objs[i].customer_id+');">'+
                    '<i class="icon-edit icon-white"></i>编辑'+
                '</a>&nbsp;'+
                '<a class="btn btn-danger" href="javascript:void(0);" onclick="deleteCompanyCreditGrant('+objs[i].id+');">'+
                    '<i class="icon-trash icon-white"></i>删除'+
                '</a>'+ 
            '</div>';
            $('#yhTable').dataTable().fnAddData( [
              '<input type="checkbox" name="company_credit_grant_checkbox" onclick="changeColor(this)"/>',
              objs[i].customer_number,
              objs[i].company_name,
              content
              ]
...
function gotoBjCompanyCreditGrant(id,customer_id){
    var param = {
        "id":id,
        "customer_id":customer_id
    }
...
function initCompanyCreditGrantContent(){
    var param = $.evalJSON($("#jsonparam",window.parent.document).val());
...
function deleteCompanyCreditGrant(id){
    var del = crud.dom.factory("DELETE");
    var url = "/grantcredit/" + id;
    del.doDelete(url,initSalerContentCallback,"删除失败！");
    
    function initSalerContentCallback(json){
        window.location.href = "company_creditgrant";
    }
}

function getAllCompanyCreditManage(){
    var get = crud.dom.factory("GET");
    var url = "/managecredit/list?credit_type=2";
    get.doGet(url,getAllUserCallback,"获取贷款信息失败！");
    ...
    function getAllUserCallback(json){
        ...
        for(var i = 0;i<objs.length;i++){
            var content = '<div>'+
                '<a class="btn btn-info" href="javascript:void(0);" onclick="gotoBjCompanyCreditmanage('+objs[i].id+','+objs[i].customer_id+');">'+
                    '<i class="icon-edit icon-white"></i>编辑'+
                '</a>&nbsp;'+
                '<a class="btn btn-danger" href="javascript:void(0);" onclick="deleteCompanyCreditmanage('+objs[i].id+');">'+
                    '<i class="icon-trash icon-white"></i>删除'+
                '</a>'+ 
            '</div>';
            $('#yhTable').dataTable().fnAddData( [
              '<input type="checkbox" name="company_credit_manage_checkbox" onclick="changeColor(this)"/>',
              objs[i].customer_number,
              objs[i].company_name,
              content
              ]
...
function gotoBjCompanyCreditmanage(id,customer_id){
    var param = {
        "id":id,
        "customer_id":customer_id
    }
...
function initCompanyCreditManegeContent(){
    var param = $.evalJSON($("#jsonparam",window.parent.document).val());
...
function deleteCompanyCreditmanage(id){
    var del = crud.dom.factory("DELETE");
    var url = "/managecredit/" + id;
    del.doDelete(url,initSalerContentCallback,"删除失败！");
    
    function initSalerContentCallback(json){
        window.location.href = "company_creditmanage";
    }
}